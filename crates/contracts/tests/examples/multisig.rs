...
async fn test_multisig_replay_protection() -> anyhow::Result<()> {
    ...
    match result {
        Err(TransactionExecutorError::TransactionProgramExecutionFailed(err)) => {
            // Verify the error is related to the transaction already being executed
            let err_str = format!("{err:?}");
            assert!(
                err_str.contains(&format!("{}", ERR_TX_ALREADY_EXECUTED.code())),
                "Expected ERR_TX_ALREADY_EXECUTED error, got: {err_str}"
            );
        }
        Ok(_) => panic!("Expected replay protection to fail the transaction"),
...
async fn test_multisig_update_signers_remove_owner() -> anyhow::Result<()> {
    ...
    assert_eq!(
        extracted_pub_keys.len(),
        2,
        "Should have 2 public keys after update"
    );
    ...
    for i in 0..5 {
        ...
        if storage_item != Word::default() {
            ...
            assert!(
                i < 2,
                "Found non-empty key at index {} which should be removed",
                i
            );
...
async fn test_multisig_new_approvers_cannot_sign_before_update() -> anyhow::Result<()> {
    ...
    let new_sig_1 = new_authenticators[0]
        .get_signature(
            new_public_keys[0].to_commitment().into(),
            &tx_summary_signing,
        )
        .await?;
    let new_sig_2 = new_authenticators[1]
        .get_signature(
            new_public_keys[1].to_commitment().into(),
            &tx_summary_signing,
        )
        .await?;
...
async fn test_check_note_consumability_multisig() -> anyhow::Result<()> {
    ...
    let block_ref = tx_context_without_signatures
        .tx_inputs()
        .block_header()
        .block_num();
    let tx_args = tx_context_without_signatures.tx_args();
    let tx_executor =
        TransactionExecutor::<'_, '_, _, BasicAuthenticator>::new(&tx_context_without_signatures);
    ...
    // Note: We don't add p2id_note as an expected output - it's an INPUT note being consumed
    ...
    // Verify the transaction actually executes successfully with signatures
    let executed_tx = tx_context_with_signatures.execute().await?;
    assert_eq!(
        executed_tx.account_delta().nonce_delta(),
        Felt::new(1),
        "Transaction should execute successfully with signatures"
    );

    // Note: We can't test can_consume returning `Consumable` for multisig accounts because:
    // 1. can_consume internally creates fresh TransactionInputs and executes the transaction
    // 2. This execution computes its own TX_SUMMARY_COMMITMENT
    // 3. Pre-computed signatures won't match because they were signed over a different tx_summary
    //
    // For multisig, can_consume will always return `ConsumableWithAuthorization` when called with
    // pre-computed signatures, since the signatures don't match the internally computed tx_summary.
    // The test above (execute()) verifies that multisig auth works when signatures match.
...
async fn test_multisig_proc_threshold_overrides() -> anyhow::Result<()> {
    ...
    assert!(
        tx_result.is_ok(),
        "Note consumption with 1 signature should succeed"
    );
    ...
    assert!(
        result.is_ok(),
        "Transaction should succeed with 2 signatures for note sending"
    );
